...
function process_order_item_for_subscription($subscription, $process_type){
    ...
    if ($subscription && isset($subscription->order_type) && $subscription->order_type == 'shop_subscription') {
        ...
        //set license key in order detail metabox
        //$licenseNumber = my_custom_checkout_field_display_admin_order_meta($subscription);
        ...
        foreach ($subscription->get_items() as $item_id => $item) {
            ...
            {   
                for($i = 0; $i<count($prod_acct_fields); $i++)
                {
                    ...
                    {
                        $account_info[STRIKEFORCE][$prod_acct_fields[$i]] = get_post_meta( $product_id, $prod_acct_fields[$i], true );
                    }
                }

                $guardedIdApi = get_guardedID_manager($account_info[STRIKEFORCE]);
            }
            ...
            switch ($process_type) {
                ...
                case 'hold':
                    ...
                    {
                        $subscription->add_order_note($reasonForChange. ' StrikeForce API update a '. strtolower($enableResponse['ErrorDescription']) . '.', 0, false);
                        do_action('license_suspension_procedure', $subscription_id, 0, $account_info[STRIKEFORCE]);
                    } else {
                        $subscription->add_order_note($reasonForChange. ' StrikeForce API not updated successfully.', 0, false);
                    }
                ...
                case 'activate':
                    if (isset($item['License'])) {
                        ...
                        if ($enableResponse["ErrorDescription"] == "Success")
                        {
                            $subscription->add_order_note($reasonForChange. ' StrikeForce API update '. strtolower($enableResponse['ErrorDescription']) . '.', 0, false);
                        } else {
                            $subscription->add_order_note($reasonForChange. ' StrikeForce API not updated successfully.', 0, false);
                        }
 ...
 */
// function get_license_status_for_subscription($subscription_key){
...
function convertXml2Array ( $xmlObject, $out = array () )
{
    foreach ( (array) $xmlObject as $index => $node )
        $out[$index] = ( is_object ( $node ) ) ? convertXml2Array ( $node ) : $node;
